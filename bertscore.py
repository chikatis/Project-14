        ...
        if not precise_alignments.empty:
            ...
            if (not train) and (model_name is not None) and (pt is not None):
                ...
                if misalignments:
                    ...
                    all_misalignments_df.to_csv(f'./bertscore-data/misalignments/{pt}/{pt} {model} Misalignments.csv', index=False)

                if misalignments_test:
                    all_misalignments_test_df = pd.concat(misalignments_test, ignore_index=True)
                    all_misalignments_test_df.to_csv(f'./bertscore-data/misalignments/{pt}/{pt} {model} Misalignments Test.csv', index=False)
...
def implement_model(model_name):
    ...
    graph_dir = './bertscore-data/graphs'
    ...
    for pt, (national_threshold, pt_threshold) in province_territories.items():
        ...
        bert_scores = generate_embeddings(national_df, pt_df, model_name)
        ...
        similarity_df = get_matchings(bert_scores, national_df, pt_df)
        ...
        plt.title(f'BERTScore AER vs. Threshold for {pt}', fontsize=14)
...
def save_outputs(best_threshold, test_aers, test_alignments, model_name):
    ...
    output_file = f'./bertscore-data/results/{model_name.split("/")[-1]} Results.json'
...
bertscore_best_threshold, bertscore_test_aers, bertscore_test_alignments = implement_model('BERTScore')

# Save the results to a JSON file
save_outputs(bertscore_best_threshold, bertscore_test_aers, bertscore_test_alignments, 'BERTScore')
