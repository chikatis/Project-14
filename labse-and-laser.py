...
# Path to BERT-Scores
labse_scores_path = './labse-data/scores'
laser_scores_path = './laser-data/scores'
...
def implement_model(model_name):
    ...
    if model_name == 'LaBSE':
        main_scores_path = labse_scores_path
    elif model_name == 'LASER':
        main_scores_path = laser_scores_path
...
labse_test_alignments, labse_aers, labse_results = implement_model('LaBSE')

# Save all the alignments from BERTScore
for key, value in labse_test_alignments.items():
    value.to_csv(f'./labse-data/results/alignments/{key} Alignments.csv', index=False)


laser_test_alignments, laser_aer, laser_results = implement_model('LASER')
# Save all the alignments from the LASER model
for key, value in laser_test_alignments.items():
    value.to_csv(f'./laser-data/results/alignments/{key} Alignments.csv', index=False)
